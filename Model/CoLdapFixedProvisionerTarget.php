...
class CoLdapFixedProvisionerTarget extends CoProvisionerPluginTarget {
      ...
      if(  $supportedAttributes[$oc]['objectclass']['required']
        ...
        foreach(array_keys($supportedAttributes[$oc]['attributes']) as $attr) {
          if($supportedAttributes[$oc]['attributes'][$attr]['required']
            ...
            $targetType = isset($configuredAttributes[$oc][$attr]) && isset($configuredAttributes[$oc][$attr]['type']) ? $configuredAttributes[$oc][$attr]['type'] : "";
            switch($attr) {
              // Name attributes
              case 'cn':
                if($person) {
  ...
  public function provision($coProvisioningTargetData, $op, $provisioningData) {
    ...
    try {
      $dns = $this->CoLdapFixedProvisionerDn->obtainDn($coProvisioningTargetData,
                                                       $provisioningData,
                                                       $person ? 'person' : 'group',
                                                       $assigndn);
    }
    ...
    if($dns['newdn'] && ($add || $modify)) {
      ...
      try {
        $dnAttributes = $this->CoLdapFixedProvisionerDn->dnAttributes($coProvisioningTargetData,
                                                                      $dns['newdn'],
                                                                      $person ? 'person' : 'group');
      }
      catch(RuntimeException $e) {
...
//                                              $dns['newdnerr'])));
  ...
  protected function queryLdap($serverUrl,$bindDn,$password, $baseDn, $filter, $attributes=array()) {
    ...
    if(!$s) {
      throw new RuntimeException($this->ldap_error() . " (" . $baseDn . ")", $this->ldap_errno());
    }
